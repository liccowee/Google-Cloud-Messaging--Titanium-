...
#define TAG "com.activate.gcm"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Gcm_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Gcm.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::GcmBindings::lookupGeneratedInit(
...
static void Gcm_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Gcm_getBinding)->GetFunction());
}

static void Gcm_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::GcmBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry GcmBinding = {
	"com.activate.gcm",
	Gcm_init,
	Gcm_dispose
};
...
Java_com_activate_gcm_GcmBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.activate.gcm", &GcmBinding);
	titanium::KrollBindings::addExternalLookup(&(::GcmBindings::lookupGeneratedInit));
}
